...
class ObjectCounter(BaseSolution):
    ...
    def count_objects(self, current_centroid, track_id, prev_position, cls, box):
        """
    ...
    def count(self, im0):
        ...
        for box, track_id, cls in zip(self.boxes, self.track_ids, self.clss):
            ...
            self.count_objects(current_centroid, track_id, prev_position, cls, box)  # Perform object counting

        self.display_counts(im0)  # Display the counts on the frame
        self.display_output(im0)  # display output with base class function

        return im0,self.stoneDict  # return output image for more usage

    def CountBox(self, box, track_id):
        if track_id not in self.counted_ids:
            x1, y1, x2, y2 = box
            stone_scale = ((x2-x1)+(y2-y1))/2
            # stone_scale = max(x1, y1, x2, y2)
            self.stoneDict[track_id] = stone_scale.item()
